...
import es.uji.ei1027.sgOvi.model.Nadador;
...
public class NadadorDao {
    ...
    public void deleteNadador(String nom) {
        jdbcTemplate.update("DELETE FROM Nadador WHERE nom LIKE " + nom + "'");
    }

    /* Actualitza els atributs del nadador
       (excepte el nom, que és la clau primària) */
    public void updateNadador(Nadador nadador) {
        jdbcTemplate.update("UPDATE Nadador SET edat = "+ nadador.getEdat() + ", num_federat = " + nadador.getNumFederat() +",pais = "+ nadador.getPais()+ ",edat = " + nadador.getEdat() + ", genere = " + nadador.getGenere() + " WHERE nom LIKE " + nadador.getNom() + "'");
    }
    ...
    public Nadador getNadador(String nomNadador) {
        try {
            Nadador n = jdbcTemplate.queryForObject(
                    "SELECT * FROM Nadador WHERE nom = '" + nomNadador + "'",
